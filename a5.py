...
class Board:
    ...
    def find_most_constrained_cell(self) -> Tuple[int, int]:
        ...
        for col in range(self.size):
            for row in range(self.size):
                ...
                if isinstance (cell, list) and len (cell) < minSize:
    ...
    def update(self, row: int, column: int, assignment: int) -> None:
        ...
        if isinstance(self.rows[row][column], list) and assignment in self.rows[row][column]:
            ...
            self.numNumsPlaced += 1

            # Remove assigned value from the same row, column, and subgrid
            for r in range(self.size):
                remove_if_exists(self.rows[r][column], assignment)  # Remove from the same column
                remove_if_exists(self.rows[row][r], assignment)     # Remove from the same row
            
            subgrid_coords = self.subgrid_coordinates(row, column)
            for sub_row, sub_col in subgrid_coords:
                remove_if_exists(self.rows[sub_row][sub_col], assignment)  # Remove from the same subgrid


def DFS(state: Board): #depthFirstSearch
    ...
    while not stack.isEmpty():
        ...
        if cell is not None:
                row, col = cell
                for value in current_board.rows[row][col]:
...
def BFS(state: Board) -> Board: #breadthFirstSearch
    ...
    while not queue.is_empty():
        ...
        cell = current_board.find_most_consrained_cell()
        if cell is not None:
             row, col = cell
             for value in current_board.rows[row][col]:
                 new_board = copy.deepcopy(current_board)
